...
import { getExchangeRateToStoreExpenses, editTableRow } from '../actions';
...
class Wallet extends React.Component {
  ...
  handleSaveExpenses() {
    const { editing, expenses, oldExpenses } = this.state;
    const { getExpenses, editExpenseDataInStore } = this.props;

    if (editing) {
      editExpenseDataInStore(expenses);
      this.setState({
  ...
  render() {
    ...
    const { currencies, expenses, editing } = this.state;
...
const mapDispatchToProps = (dispatch) => ({
  ...
  editExpenseDataInStore: (expense) => dispatch(editTableRow(expense)),
});
...
Wallet.propTypes = {
  ...
  editExpenseDataInStore: PropTypes.func.isRequired,
};
